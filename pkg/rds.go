...
func (e *RDSExporter) addAllInstanceMetrics(configIndex int, instances []rds_types.DBInstance, eolInfos []EOLInfo) {
	...
	for _, instance := range instances {
		...
		e.cache.AddMetric(prometheus.MustNewConstMetric(AllocatedStorage, prometheus.GaugeValue, float64(*instance.AllocatedStorage*1024*1024*1024), e.getRegion(configIndex), *instance.DBInstanceIdentifier))
