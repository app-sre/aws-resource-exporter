...
func (e *RDSExporter) getRDSLogMetrics(index int, instanceId string, instanceTags string) error {
	...
	e.cache.AddMetric(prometheus.MustNewConstMetric(e.LogsAmount, prometheus.GaugeValue, float64(logMetrics.logs), *e.sessions[index].Config.Region, instanceId, instanceTags))
	e.cache.AddMetric(prometheus.MustNewConstMetric(e.LogsStorageSize, prometheus.GaugeValue, float64(logMetrics.totalLogSize), *e.sessions[index].Config.Region, instanceId, instanceTags))
	return nil
}

func (e *RDSExporter) createWorkerPool(index int, instancesQueue <-chan string, instancesTags map[string]string) (context.Context, context.CancelFunc) {
		...
		go func(queue <-chan string, wg *sync.WaitGroup) {
			...
			for {
				...
				case instanceId, ok := <-queue:
					...
					err := e.getRDSLogMetrics(index, instanceId, instancesTags[instanceId])
					if err != nil {
						cancel()
						return
					}
...
func (e *RDSExporter) CollectLoop() {
	for {
		for i, _ := range e.sessions {
			...
			ctx, cancel := e.createWorkerPool(i, instancesQueue, instances_tags)
			defer cancel()
			...
			for _, instance := range instances {
				...
				if valmap, ok := DBMaxConnections[*instance.DBInstanceClass]; ok {
					...
					if found {
						...
						e.cache.AddMetric(prometheus.MustNewConstMetric(e.MaxConnectionsMappingError, prometheus.GaugeValue, 0, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, *instance.DBInstanceClass, instances_tags[*instance.DBInstanceIdentifier]))
					} else {
						level.Error(e.logger).Log("msg", "No DB max_connections mapping exists for instance",
							"type", *instance.DBInstanceClass,
							"group", *instance.DBParameterGroups[0].DBParameterGroupName)
						e.cache.AddMetric(prometheus.MustNewConstMetric(e.MaxConnectionsMappingError, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, *instance.DBInstanceClass, instances_tags[*instance.DBInstanceIdentifier]))
					}
				} else {
					level.Error(e.logger).Log("msg", "No DB max_connections mapping exists for instance",
						"type", *instance.DBInstanceClass)
				...
				if *instance.PubliclyAccessible {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.PubliclyAccessible, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))

				} else {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.PubliclyAccessible, prometheus.GaugeValue, 0, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))

				}

				if *instance.StorageEncrypted {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.StorageEncrypted, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))

				} else {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.StorageEncrypted, prometheus.GaugeValue, 0, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))

				}

				if instance.LatestRestorableTime != nil {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.LatestRestorableTime, prometheus.CounterValue, float64(instance.LatestRestorableTime.Unix()), *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))
				} else {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.LatestRestorableTime, prometheus.CounterValue, float64(0), *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))
				}

				e.cache.AddMetric(prometheus.MustNewConstMetric(e.MaxConnections, prometheus.GaugeValue, float64(maxConnections), *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))
				e.cache.AddMetric(prometheus.MustNewConstMetric(e.AllocatedStorage, prometheus.GaugeValue, float64(*instance.AllocatedStorage*1024*1024*1024), *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, instances_tags[*instance.DBInstanceIdentifier]))
				e.cache.AddMetric(prometheus.MustNewConstMetric(e.DBInstanceStatus, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, *instance.DBInstanceStatus, instances_tags[*instance.DBInstanceIdentifier]))
				e.cache.AddMetric(prometheus.MustNewConstMetric(e.EngineVersion, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, *instance.Engine, *instance.EngineVersion, instances_tags[*instance.DBInstanceIdentifier]))
				e.cache.AddMetric(prometheus.MustNewConstMetric(e.DBInstanceClass, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, *instance.DBInstanceClass, instances_tags[*instance.DBInstanceIdentifier]))
			...
			for _, instance := range instancesPendMaintActionsData {
				for _, action := range instance.PendingMaintenanceActionDetails {
					...
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.PendingMaintenanceActions, prometheus.GaugeValue, 1, *e.sessions[i].Config.Region, dbIdentifier, *action.Action, autoApplyDate, currentApplyDate, *action.Description, instances_tags[dbIdentifier]))
				}
			}
			...
			for _, instance := range instances {
				if !instancesWithPendingMaint[*instance.DBInstanceIdentifier] {
					e.cache.AddMetric(prometheus.MustNewConstMetric(e.PendingMaintenanceActions, prometheus.GaugeValue, 0, *e.sessions[i].Config.Region, *instance.DBInstanceIdentifier, "", "", "", "", instances_tags[*instance.DBInstanceIdentifier]))
				}
			}
		}
